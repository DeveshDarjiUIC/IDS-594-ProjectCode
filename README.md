...
```bash
pip install pandas scikit-learn prefect skops flask
```
...
1. Prepare your dataset as `train.csv` with the following expected columns:
   - `id` (index column)
...
1. Ensure `bank_model.skops` exists in the same directory
...
4. Paste CSV data in the text area with properly formatted features (matching training data columns)

5. Click "Predict" to get churn predictions
